...
class BaseNavigationController: UINavigationController, UINavigationControllerDelegate {
    ...
    private func styleiOS14NavBar() {
        if #available(iOS 15.0, *) {
            ...
            appearance.backgroundColor = UIColor.appBar(for: traitCollection)
            appearance.titleTextAttributes = navTitleTextAttributes(for: traitCollection)
            navigationBar.standardAppearance = appearance;
            navigationBar.scrollEdgeAppearance = navigationBar.standardAppearance
            navigationBar.tintColor = UIColor.appMain(for: traitCollection)
        }
    }
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        let backImage = UIImage(named: "Back")?.maskedWithColor(UIColor.appMain(for: traitCollection))
